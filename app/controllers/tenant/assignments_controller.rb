class Tenant::AssignmentsController < Tenant::BaseController
  ...
  def edit
    if params[:instance_start].present? and params[:instance_end].present?
      ...
      if params[:day_delta].present? && params[:minute_delta].present?
        ...
        @resource.overrided_start_at  = DateTime.parse(params[:instance_start]).to_time.advance(days: d, minutes: m).utc.iso8601
      else
        @resource.overrided_start_at  = DateTime.parse(params[:instance_start]).to_time.utc.iso8601
      end
    else
  ...
  #     :recurrence_action, :recurrence_action_type, :recurrence, :recurring_until_at, :overrided_start_at)
